...
st.title("🔮 美股量化投資沙盒完全體 (環境切換 + 雲端清單對比強化版)")
...
def run_backtest_engine(df, strategy_name, days, posture):
    ...
    if "🚀 大膽進攻型" in posture:
        rsi_max, vol_mult, dip_pct, rsi_min, chip_col = 75, 1.05, -0.10, 35, '主力籌碼_Q80'
    elif "🥶 極度謹慎型" in posture:
        rsi_max, vol_mult, dip_pct, rsi_min, chip_col = 65, 1.50, -0.15, 25, '主力籌碼_Q95'
    else: 
        rsi_max, vol_mult, dip_pct, rsi_min, chip_col = 70, 1.20, -0.10, 30, '主力籌碼_Q90'
...
with tab_debug:
    ...
    if st.session_state.calculated:
        ...
        if db_key in st.session_state.detail_db:
            ...
            if len(buys) > 0:
                fig.add_trace(go.Scatter(x=[b[0] for b in buys], y=[b[1] for b in buys], mode='markers', name='🟢 BUY (進場)', marker=dict(symbol='triangle-up', size=12, color='#00FF00'), hovertemplate='<b>進場買點</b><br>日期: %{x|%Y-%m-%d}<br>價格: $%{y:.2f}<br>當日RSI: %{customdata[0]:.1f}<extra></extra>', customdata=[[float(v_df.loc[b[0], 'RSI_14'])] for b in buys]))
            if len(sells) > 0:
